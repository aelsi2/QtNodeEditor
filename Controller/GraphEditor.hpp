...
typedef  QSet<QUuid> NodeCollection;
...
public:
    ...
    virtual void move(NodeCollection const &nodes, QPointF delta);
    
    virtual QUuid createNode(NodeType type, QPointF position = QPointF(0, 0));
    virtual void deleteNodes(NodeCollection const &nodes);
    
    virtual void cut(NodeCollection const &nodes);
    virtual void copy(NodeCollection const &nodes);
    virtual void pasteClipboard();
    virtual void duplicate(NodeCollection const &nodesInput);
...
signals:
    ...
    void requestDeselect(QUuid nodeId);
...
protected:
    ...
    void doSelectNode(QUuid nodeId);
    void doDeselectNode(QUuid nodeId);
